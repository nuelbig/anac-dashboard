...
const buildPhotoUrl = (photoUrl: string): string => {
  ...
  // En développement, utiliser le proxy Vite pour les chemins qui commencent par /uploads ou /api
  if (isDev) {
    // Si le chemin commence par /uploads, utiliser le proxy directement
    if (photoUrl.startsWith("/uploads")) {
      console.log("🔗 URL via proxy /uploads (dev):", photoUrl);
      return photoUrl;
    }
    // Si le chemin commence par /api/v1/uploads, utiliser le proxy
    if (photoUrl.startsWith("/api/v1/uploads") || photoUrl.startsWith("/api/uploads")) {
      console.log("🔗 URL via proxy /api (dev):", photoUrl);
      return photoUrl;
    }
  }

  // En production ou si le chemin ne correspond pas aux patterns ci-dessus
  // Construire l'URL complète avec SERVER_URL
  const baseUrl = SERVER_URL.endsWith("/") ? SERVER_URL.slice(0, -1) : SERVER_URL;
  const path = photoUrl.startsWith("/") ? photoUrl : `/${photoUrl}`;
  const fullUrl = `${baseUrl}${path}`;
  
  console.log("🌐 URL construite:", fullUrl, "(photoUrl original:", photoUrl, ", DEV:", isDev, ")");
  return fullUrl;
};
